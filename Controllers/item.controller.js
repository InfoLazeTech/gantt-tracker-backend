    ...
    if (processes && Array.isArray(processes)) {
      for (const proc of processes) {
        if (!mongoose.Types.ObjectId.isValid(proc.processId)) {
          return res
            .status(400)
            .json({ error: "Invalid processObjId in processes." });
        }
      }
    }
    const item = new Item({
...
exports.updateItem = async (req, res) => {
  try {
    ...
    // Preserve old startDateTime / endDateTime for each process
    const updatedProcesses = processes.map((newProc) => {
      const oldProc = existingItem.processes.find(
        (p) => p.processId.toString() === newProc.processId
      );

      return {
        ...newProc,
        startDateTime: oldProc?.startDateTime || null,
        endDateTime: oldProc?.endDateTime || null,
      };
    });
    ...
    const updatedItem = await Item.findOneAndUpdate(
      { itemId },
      {
        ...
        description,
        recipeId,
        processes: updatedProcesses, // ✅ FIXED HERE
        updatedAt: new Date(),
      },
    ...
    if (!updatedItem) {
      return res.status(404).json({ error: "Item not found during update." });
    }
...
exports.getItemByReference = async (req, res) => {
  try {
    ...
    if (!PONumber && !RefNumber) {
      return res
        .status(400)
        .json({ message: "PONumber or RefNumber is required." });
    }
